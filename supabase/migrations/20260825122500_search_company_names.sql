-- monadic — let search match company names
--
-- The company filter is being removed from the panel in favour of searching for
-- a company by name. That only works if search actually looks at the company
-- name, which it did not: matching was title-only, with descriptions as an
-- opt-in. Typing "Ramp" found roles with "ramp" in the title and missed every
-- role at Ramp.
--
-- Company name joins title as a primary matching field, weighted below an exact
-- title hit but above a stemmed one — searching "engineer" should still rank
-- engineering roles above every role at a company with "engineer" in its name.
...
as $$
  ...
  matched as (
    select f.*, j.location_cities as cities,
      case when (select term from q) is null then 0::real
          ...
          -- A company-name hit is a strong signal but must not outrank a title
          -- match: searching "engineer" should surface engineering roles before
          -- every role at a company whose name contains "engineer".
          + (case when f.company_name ilike '%' || (select term from q) || '%' then 2 else 0 end)
    ...
    where f.is_open
      ...
      and ((select term from q) is null
           ...
           or f.company_name ilike '%' || (select term from q) || '%'
  ...
  order by
    ...
    r.posted_at desc nulls last, r.first_seen_at desc, r.id
