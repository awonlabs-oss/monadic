...
async function main() {
  ...
  for (const company of companies) {
    ...
    if (logo && logo.width < FLOOR_PX) {
      // Explicitly cleared, not skipped: --force has to be able to undo a bad
      // resolution from a previous run, or the mush is permanent.
      const { error: clearError } = await db
        .from("companies")
        .update({ logo_url: null })
        .eq("id", company.id);
      if (clearError) throw new Error(`${company.slug}: ${clearError.message}`);
      console.log(
        `  ${String(logo.width) + "px"} too small  ${company.name.padEnd(18)} monogram is crisper`,
      );
    } else if (logo) {
      found += 1;
      if (logo.width < MIN_PX) tiny += 1;
      const { error: updateError } = await db
        .from("companies")
        .update({ logo_url: logo.href })
        .eq("id", company.id);
      if (updateError) throw new Error(`${company.slug}: ${updateError.message}`);
      const size = logo.width === Number.POSITIVE_INFINITY ? "vector" : `${logo.width}px`;
      const flag = logo.width < MIN_PX ? " (under 64px — will look soft)" : "";
      console.log(`  ${size.padEnd(7)} ${company.name.padEnd(18)} ${logo.href.slice(0, 62)}${flag}`);
    } else {
